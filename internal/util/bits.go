...
func ConcurrentColumnarBitSetSymmetricDifference(matrix []*bitset.BitSet, f func(int) *bitset.BitSet) []*bitset.BitSet {
	...
	for i := 0; i < nThreads; i++ {
		...
		go func(i int) {
			...
			for c := 0; c < (nColumns + extraColumns); c++ {
				// this bitset is generated by an external function (really a hashing function in real implementation)
				genset := f(c)
				row := bitset.New(uint(m))
				for r := 0; r < m; r++ {
					// Take XOR here
					if matrix[r].Test(uint((i*nColumns)+c)) != genset.Test(uint(r)) {
						row.Set(uint(r))
					}
				}
