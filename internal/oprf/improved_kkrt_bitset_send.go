...
type imprvKKRTBitSetSend struct {
...
func NewImprovedKKRTBitSetSend(m, k, baseOT int, ristretto bool) (OPRF, error) {
	...
	if err != nil {
		return imprvKKRTBitSetSend{}, err
	}
	g := blake3.New()

	return imprvKKRTBitSetSend{baseOT: ot, m: m, k: k, prng: rand.New(rand.NewSource(time.Now().UnixNano())), g: g}, nil
}

// Send returns the OPRF keys
func (ext imprvKKRTBitSetSend) Send(rw io.ReadWriter) (keys []Key, err error) {
	start := time.Now()
	...
	ub := bitset.New(uint(ext.m))
	q := make([][]byte, ext.k)
	for col := range q {
		if _, err = ub.ReadFrom(rw); err != nil {
			return nil, err
		}
		u := util.BitSetToBits(ub)
...
func (ext imprvKKRTBitSetSend) Receive(choices [][]byte, rw io.ReadWriter) (t [][]byte, err error) {
	...
	for col := range d {
		...
		// send u
		if _, err = util.BitsToBitSet(u).WriteTo(rw); err != nil {
			return nil, err
		}
	}

	return util.ContiguousTranspose(t), nil
}

// Encode computes and returns OPRF(k, in)
func (o imprvKKRTBitSetSend) Encode(k Key, in []byte) (out []byte, err error) {
	// compute q_i ^ (C(r) & s)
	x, err := util.AndBytes(crypto.PseudorandomCode(k.sk, in), k.s)
	if err != nil {
		return
	}
